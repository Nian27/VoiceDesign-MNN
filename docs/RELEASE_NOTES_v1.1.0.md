...
| 音色创建闭环（设计 + 注册）·**同会话 A/B** | **245.6 s**（参考文本 26 字，`steps=91`，decode 188.9 s） | **153.5 s**（15 字，`steps=51`，decode 106.2 s） | **1.60×**（步数 1.78×） |
| 同上 · 设备较凉时的另一次实测 | — | **97.3 s**（prefill 19.4 / decode 61.5 / decoder 6.6 / enroll 6.4） | **2.52×** |
| 参考音频**有效保留率** | 设计 7.28 s，被 5 秒上限砍到 5.00 s（保留 **69%**） | 设计 4.08 s，**100% 保留** | 少丢 2.28 s |
...
> 口径声明：音色创建那几行是**同一台设备、同一描述（「萝莉」）、同一后端（opencl）、同一会话**下的 A/B 实测 ——
> 唯一变量是默认参考文本从 26 字改成 15 字（自回归步数 `steps=91 → 51`，decode 188.9 s → 106.2 s）。
> 97.3 s 那次是**另一时段设备较凉**时的实测，与 153.5 s 的差异（1.58×）来自设备状态而不是代码，
> 所以两个倍数都列出，不取最好看的那个。其余各行都是同一台设备、同一句文本的实测值。
