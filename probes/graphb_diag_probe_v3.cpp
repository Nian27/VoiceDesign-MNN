// graphb_diag_probe_v3.cpp - 7 inputs / 19 outputs (host rope + host mask + exposed attn internals)
// usage: graphb_diag_probe_v3 MODEL EMB CP KVK KVV COS SIN MASK OUT_DIR [cpu|htp]
...
int main(int argc, char** argv) {
    if (argc < 11) { fprintf(stderr, "usage: %s MODEL EMB CP KVK KVV COS SIN MASK OUT_DIR [cpu|htp]\n", argv[0]); return 2; }
    ...
    std::vector<std::string> ins = {"inputs_embeds","cache_position","kv_k","kv_v","rope_cos","rope_sin","attn_mask"};
    std::vector<std::string> outs = {"norm_out","q_raw","k_raw","v_raw","q_norm","k_norm","q_rope","k_rope","qk_logits","qk_masked","attn_probs",
    ...
    for (size_t i = 0; i < out.size(); i++) {
        ...
        double mx = 0; bool fin = true; size_t ninf = 0;
        for (size_t k = 0; k < elems; k++) { float x = d[k]; if (x != x) fin = false; if (x < -1e37f) ninf++; double a = x < 0 ? -x : x; if (a > mx && a < 1e37) mx = a; }
        printf("OUT[%zu] %-11s elems=%zu maxabs=%.6f finite=%d neginf=%zu\n", i, i<outs.size()?outs[i].c_str():"?", elems, mx, fin?1:0, ninf);
