// g2b_loop.cpp - G2-B self-driven shared-RNG trajectory, full attribution instrumentation
// usage: g2b_loop GRAPHB CODECHEAD CODECEMB CODEPRED FRAMEEMB COS300 SIN300 INIT_KVK INIT_KVV OUTDIR [cpu|htp] N CP0
...
static Pick sampleTopK(const float* lg, int vocab, double temp, int topk, bool suppress, double u) {
    ...
    for (int i = 0; i < vocab; i++) {
        if (suppress && i >= 2048 && i != 2150) continue;
...
int main(int argc, char** argv) {
    if (argc < 13) { fprintf(stderr,"usage: %s GRAPHB CODECHEAD CODECEMB CODEPRED FRAMEEMB COS SIN KVK KVV OUTDIR [cpu|htp] N CP0\n", argv[0]); return 2; }
    const char* fGB=argv[1]; const char* fCH=argv[2]; const char* fCE=argv[3]; const char* fCP=argv[4]; const char* fFE=argv[5];
    std::vector<char> cosB=readFile(argv[6]), sinB=readFile(argv[7]), kvkB=readFile(argv[8]), kvvB=readFile(argv[9]);
    const char* outDir=argv[10];
    const char* backend=argc>11?argv[11]:"cpu";
    int N=argc>12?atoi(argv[12]):40;
    int CP0=argc>13?atoi(argv[13]):62;
    ...
    std::vector<float> emb(2048,0.f), maskv(KV,0.f), slotv(KV,0.f);
    std::vector<float> c0e(2048), lg(3072), lg15(15*2048), embn(2048);
    ...
    for (int n=0;n<N;n++) {
        ...
        memcpy(vCH->writeMap<float>(),hidb.data(),2048*4);
        std::vector<VARP> o2=mCH.m->onForward({vCH});
        const float* lgraw=o2[0]->readMap<float>();
        memcpy(lg.data(),lgraw,3072*4);
        tCH+=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-t1).count();
        double u0=nextU();
        Pick p0=sampleTopK(lg.data(),3072,0.9,50,true,u0);
        int code0=p0.idx;
        if (code0==2150){ stopReason=0; printf("EOS at step %d cp=%d\n",n,cp); fprintf(fr,"step %d cp %d STOP_EOS u0 %.9f\n",n,cp,u0); break; }
        ...
        memcpy(embn.data(),o5[0]->readMap<float>(),2048*4);
