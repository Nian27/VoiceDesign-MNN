// graphb_diag_probe_v2.cpp - 1L DIAG with HOST-provided merged rope cos/sin
// usage: graphb_diag_probe_v2 MODEL EMB CP KVK KVV COS SIN OUT_DIR [cpu|htp]
...
int main(int argc, char** argv) {
    if (argc < 10) { fprintf(stderr, "usage: %s MODEL EMB CP KVK KVV COS SIN OUT_DIR [cpu|htp]\n", argv[0]); return 2; }
    const char* model = argv[1];
    std::vector<std::vector<char>> raws;
    for (int i = 2; i <= 7; i++) raws.push_back(readFile(argv[i]));
    ...
    std::vector<std::string> ins = {"inputs_embeds","cache_position","kv_k","kv_v","rope_cos","rope_sin"};
    ...
    for (size_t i = 0; i < out.size(); i++) {
        ...
        double mx = 0;
        for (size_t k = 0; k < elems; k++) { double a = d[k] < 0 ? -d[k] : d[k]; if (a > mx) mx = a; }
