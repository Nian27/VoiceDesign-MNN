# P3b ORT verify: hand deploy (past/present) vs authority 20 steps
...
MAX_KV = 768
NUM_STEPS = 20
LOG = open(D + '/p3b_ort.log', 'w')
def log(s):
    LOG.write(s + '\n'); LOG.flush()
cos_fn = lambda a,b: float(np.dot(a.ravel(),b.ravel())/(np.linalg.norm(a)*np.linalg.norm(b)+1e-9))
sess = ort.InferenceSession(D + '/graphb_deploy_hand.onnx', providers=['CPUExecutionProvider'])
...
for step in range(NUM_STEPS):
    ...
    n = kvk_cur.shape[3]
    # running buffer: fill slots 0..n-2 from authority (excl current, model writes current)
    ...
    try:
        ...
        kvk_buf = out[1]; kvv_buf = out[2]  # present feeds next step
        ...
        raise
log('P3B_ORT_ALL_COS ' + str(all_cos))
print('P3B_ORT_DONE', all_cos, flush=True)