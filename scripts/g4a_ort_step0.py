...
try:
    sess = ort.InferenceSession(M22 + '/graphb_dynamic_fp32_trad.onnx', providers=['CPUExecutionProvider'])
    ...
    print('step0 cos', round(cos,6), 'h', h.shape, 'max_ort', float(np.abs(h).max()), 'max_ref', float(np.abs(ref).max()), flush=True)
