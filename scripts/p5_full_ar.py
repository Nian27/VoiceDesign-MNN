...
def cp_run(cp_in, gg):
    ...
    ot.copyToHostTensor(ho)
    return np.asarray(ho.getNumpyData()).reshape(osh).astype(np.float32)
def sample_code(logits_np, seed, suppress=False):
    torch.manual_seed(seed)
    lg = torch.from_numpy(logits_np.ravel().astype(np.float32)) / 0.9
    if suppress and lg.numel() >= 3072:
        lg[2048:3072] = float('-inf')
        lg[2150] = lg[2150]
    topv, topi = torch.topk(lg, 50)
    probs = torch.softmax(topv, dim=-1)
    pick = topi[torch.multinomial(probs, 1).item()].item()
    return int(pick)
...
cp0 = int(z0['cp'][0])
with torch.no_grad():
    tts_pad_embed = talker.text_projection(talker.get_text_embeddings()(torch.tensor([[2148]])))
...
for step in range(MAX_STEPS):
    ...
    try:
        ...
        kvk_buf = g_pull('kv_k_out')
        kvv_buf = g_pull('kv_v_out')
        lg0 = ch_run(h)
        code0 = sample_code(lg0[0,0], 42 + step, suppress=True)
        with torch.no_grad():
            pj = proj(torch.from_numpy(h)).clone().numpy().astype(np.float32)
        codes15 = [sample_code(cp_run(pj, gg)[0,0], 1000 + step*15 + gg) for gg in range(15)]
        frame = [code0] + codes15
        frame_codes.append(frame)
        traj.append({'step': step, 'cp': cp_val, 'frame': frame, 'gb_ms': round((t1-t0)*1000,1)})
        if step % 10 == 0 or step < 3:
            log('step ' + str(step) + ' cp ' + str(cp_val) + ' gb_ms ' + str(round((t1-t0)*1000,1)) + ' code0 ' + str(code0) + ' codes15 ' + str(codes15[:3]) + '...')
        if code0 == 2150:
            stop = True; stop_reason = 'eos'; log('EOS at step ' + str(step))
        else:
            with torch.no_grad():
        ...
        log('STEP ERR ' + str(step) + ': ' + str(ex)[:400])
        traceback.print_exc(file=LOG)
        raise
log('TOTAL ' + str(len(frame_codes)) + ' stop ' + stop_reason)
np.save(D + '/p5_full_codes.npy', np.array(frame_codes, dtype=np.int32))
with open(D + '/p5_full_traj.json', 'w') as f:
    f.write(json.dumps({'steps': traj, 'total': len(frame_codes), 'stop_reason': stop_reason}, indent=2))
print('P5_FULL_AR_DONE', len(frame_codes), stop_reason, flush=True)