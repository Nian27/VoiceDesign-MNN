...
B,H,T,D,Cd = 1, 8, 17, 128, 2048  # T=17 as dummy (dynamic kv_len)
...
torch.onnx.export(
    core,
    (emb, pos, kk, vv, cp_),
    ART + '/graphb_dynamic_fp32.onnx',
    input_names=['inputs_embeds','position_ids','kv_k','kv_v','cache_position'],
    output_names=['hidden_states','layer_hidden'],
    dynamic_shapes={'kv_k': {3: torch.export.Dim('kv_len')}, 'kv_v': {3: torch.export.Dim('kv_len')}},
    opset_version=18,
    dynamo=True,
)
print('GRAPHB_DYNAMIC_FP32_EXPORT_OK', flush=True)