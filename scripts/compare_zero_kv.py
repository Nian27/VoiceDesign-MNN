...
pc = np.fromfile(D + '/gb_out_pc_zerokv_f32.raw', dtype=np.float32)
dev = np.fromfile(D + '/gb_zero_out_dev_f32.raw', dtype=np.float32)
...
print('ZERO-KV PC vs DEV: cosine', round(c,6), 'maxabs_diff', float(np.max(np.abs(a-b))))
