...
cp_ = np.array([34], dtype=np.int32)
sess = ort.InferenceSession(ART + '/graphb_replay_fp32ex.onnx', providers=['CPUExecutionProvider'])
t0 = time.time()
ort_hidden, ort_layer = sess.run(None, {'inputs_embeds': emb, 'position_ids': pos, 'kv_k': kvk, 'kv_v': kvv, 'cache_position': cp64})
t1 = time.time()
...
print('M3-GRAPHB MNN vs ORT: cosine', round(cos,6), 'mae', round(mae,8), 'maxabs', round(mx,6), flush=True)
print('M33_GRAPHB_DONE', flush=True)